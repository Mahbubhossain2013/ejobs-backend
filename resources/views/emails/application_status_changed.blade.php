...
<body style="font-family: 'Segoe UI', Arial, sans-serif; background-color: #f8fafc; margin: 0; padding: 40px 20px;">
    <div style="max-width: 480px; margin: 0 auto; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.06);">
        <div style="background: linear-gradient(135deg, {{ $statusColor }}, {{ $statusColorDark }}); padding: 32px 24px; text-align: center;">
            <h1 style="color: #ffffff; font-size: 22px; font-weight: 800; margin: 0;">{{ $brandName }}</h1>
            <p style="color: rgba(255,255,255,0.85); font-size: 13px; margin: 8px 0 0;">Application {{ ucfirst($status) }}</p>
        </div>
        <div style="padding: 32px 24px;">
            ...
            @if(!empty($message))
            <div style="background: {{ $statusBg }}; border-left: 4px solid {{ $statusColor }}; border-radius: 8px; padding: 16px; margin: 0 0 24px;">
                <p style="font-size: 13px; color: {{ $statusTextColor }}; margin: 0; line-height: 1.5;">{{ $message }}</p>
            </div>
            @endif
            <a href="{{ $dashboardUrl }}" style="display: inline-block; background: {{ $statusColor }}; color: #ffffff; text-decoration: none; padding: 12px 24px; border-radius: 8px; font-weight: 600; font-size: 14px;">View Application</a>
        </div>

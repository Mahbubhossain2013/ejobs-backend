...
<body style="font-family: 'Segoe UI', Arial, sans-serif; background-color: #f8fafc; margin: 0; padding: 40px 20px;">
    <div style="max-width: 480px; margin: 0 auto; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.06);">
        <div style="background: linear-gradient(135deg, #dc2626, #ef4444); padding: 32px 24px; text-align: center;">
            ...
            <p style="color: rgba(255,255,255,0.85); font-size: 13px; margin: 8px 0 0;">Subscription Expiring Soon</p>
        </div>
        <div style="padding: 32px 24px;">
            ...
            <p style="font-size: 14px; color: #334155; line-height: 1.6; margin: 0 0 24px;">
                Your <strong>{{ $planName }}</strong> subscription will expire on <strong>{{ $expiryDate }}</strong>.
            </p>
            <div style="background: #fef2f2; border-left: 4px solid #ef4444; border-radius: 8px; padding: 16px; margin: 0 0 24px;">
                <p style="font-size: 13px; color: #991b1b; margin: 0; font-weight: 600;">After expiry, you will lose access to:</p>
                <ul style="font-size: 13px; color: #991b1b; margin: 8px 0 0; padding-left: 20px; line-height: 1.8;">
                    @foreach($features as $feature)
                    <li>{{ $feature }}</li>
                    @endforeach
                </ul>
            </div>
            <a href="{{ $renewUrl }}" style="display: inline-block; background: #dc2626; color: #ffffff; text-decoration: none; padding: 12px 24px; border-radius: 8px; font-weight: 600; font-size: 14px;">Renew Now</a>
        </div>
